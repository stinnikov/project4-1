import { Product } from "../interfaces/Product"

export const ipv4 = "http://192.168.0.16:5055"

export const prods: Product[] = [
    {
        "id": "60470940-b9fe-11ea-80c9-002590bc5b5f",
        "name": "Professional Estel XTRO EX/NY Пигмент прямого действия для волос Желтый 100мл ",
        "price": "510 руб. за шт",
        "description": "Описание отсутствует",
        "imageUrl": "https://api.hk19.ru/goods_photos/233%C2%A0481.jpg",
        "amountOfImages": 1,
        "isFavourite": false,
        "isInBasket": true,
        "amountInBasket": 1
    },
    {
        "id": "33a733e9-b9ff-11ea-80c9-002590bc5b5f",
        "name": "CUREX VOLUME CR200/SO Спрей \"Живой объем\" для всех типов волос 200мл",
        "price": "490 руб. за шт",
        "description": "Описание отсутствует",
        "imageUrl": "https://api.hk19.ru/goods_photos/233%C2%A0483.jpg",
        "amountOfImages": 1,
        "isFavourite": false,
        "isInBasket": true,
        "amountInBasket": 1
    },
    {
        "id": "038a10db-e587-11ec-80dc-002590bc5b5f",
        "name": "Табличка \"Банька\" Бацькина баня ",
        "price": "279 руб. за шт",
        "description": "Описание отсутствует",
        "imageUrl": "https://api.hk19.ru/goods_photos/274%C2%A0199.jpg",
        "amountOfImages": 1,
        "isFavourite": false,
        "isInBasket": true,
        "amountInBasket": 1
    },
    {
        "id": "08a886cc-9c79-11ed-80e2-002590bc5b5f",
        "name": "Щетка-массажер для тела 13*7,5см",
        "price": "204 руб. за шт",
        "description": "Описание отсутствует",
        "imageUrl": "https://api.hk19.ru/goods_photos/286%C2%A0726.jpg",
        "amountOfImages": 1,
        "isFavourite": false,
        "isInBasket": true,
        "amountInBasket": 1
    },
    {
        "id": "0d4fb57e-bb52-11ee-80e5-002590bc5b5f",
        "name": "Набор для бани \"Царица\" (шапка, коврик, рукавица) Бацькина баня",
        "price": "579 руб. за шт",
        "description": "Описание отсутствует",
        "imageUrl": "https://api.hk19.ru/goods_photos/300%C2%A0642.jpg",
        "amountOfImages": 1,
        "isFavourite": false,
        "isInBasket": true,
        "amountInBasket": 1
    },
    {
        "id": "1735182e-eff4-11e9-8137-0cc47adbb521",
        "name": "Мочалка для тела Clean Body (У-25)",
        "price": "128 руб. за шт",
        "description": "Описание отсутствует",
        "imageUrl": "https://api.hk19.ru/goods_photos/219%C2%A0082.jpg",
        "amountOfImages": 1,
        "isFavourite": false,
        "isInBasket": true,
        "amountInBasket": 1
    },
    {
        "id": "1b0cfe32-97ed-11ee-80e5-002590bc5b5f",
        "name": "Рукавица банная \"Баня парит силу дарит\" Бацькина баня",
        "price": "167 руб. за шт",
        "description": "Описание отсутствует",
        "imageUrl": "https://api.hk19.ru/goods_photos/300%C2%A0095.jpg",
        "amountOfImages": 1,
        "isFavourite": false,
        "isInBasket": true,
        "amountInBasket": 1
    },
    {
        "id": "1b36a680-46fd-11ea-80be-002590bc5b5f",
        "name": "Шапка банная \"Успешная, красивая, всегда неоразимая\" Бацькина баня (У-25)",
        "price": "179 руб. за шт",
        "description": "Описание отсутствует",
        "imageUrl": "https://api.hk19.ru/goods_photos/226%C2%A0739.jpg",
        "amountOfImages": 1,
        "isFavourite": false,
        "isInBasket": true,
        "amountInBasket": 1
    },
    {
        "id": "22bbeda2-f469-11ed-80e3-002590bc5b5f",
        "name": "Мочалка для тела массажная силиконовая",
        "price": "131 руб. за шт",
        "description": "Описание отсутствует",
        "imageUrl": "https://api.hk19.ru/goods_photos/291%C2%A0167.jpg",
        "amountOfImages": 1,
        "isFavourite": false,
        "isInBasket": true,
        "amountInBasket": 1
    },
    {
        "id": "020f6917-3954-11ed-80e1-002590bc5b5f",
        "name": "Запарка для бани в мешочке  30г Мята Бацькина баня",
        "price": "113 руб. за шт",
        "description": "Описание отсутствует",
        "imageUrl": "https://api.hk19.ru/goods_photos/281%C2%A0742.jpg",
        "amountOfImages": 1,
        "isFavourite": false,
        "isInBasket": true,
        "amountInBasket": 1
    },
    {
        "id": "05eb839d-221e-463f-a856-68f60778c06b",
        "name": "Колготки OMSA Attiva 40 (nero 2) ",
        "price": "441 руб. за шт",
        "description": "Описание отсутствует",
        "imageUrl": "https://api.hk19.ru/goods_photos/115%C2%A0124.jpg",
        "amountOfImages": 1,
        "isFavourite": false,
        "isInBasket": true,
        "amountInBasket": 1
    }
]

export default () => { }
